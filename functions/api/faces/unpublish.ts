 ...
 *
 * The index is written before the object is deleted, which is publish.ts's
 * order reversed for publish.ts's reason: interrupted between the two leaves an
 * unlisted object, which nothing can reach and the next publish overwrites.
 *
...
export async function onRequestPost(
  ...
  await env.FACES.delete(kitKey(body.id));
