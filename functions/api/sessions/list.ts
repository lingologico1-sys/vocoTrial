 ...
 *
 * IT USED TO CARRY ONE MORE THING: which tutor protocol the stored prompt was
 * composed against, so the teacher's list could mark the codes that had gone
 * stale and tell them to publish those again. Nothing goes stale now — the
 * prompt is composed when the student dials rather than when the teacher
 * publishes — so the field went with the fault it described. See tutorPrompt.ts.
 *
