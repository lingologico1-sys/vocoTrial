...
export const SLOTS: Slot[] = [
   ...
   */
  {
    ...
    region: 'mouth',
    prompt: mouth(
      'Rest the upper front teeth directly on the lower lip, as when beginning to say "f". Compared with a relaxed closed mouth, the change is that a white band of upper teeth is now plainly visible where there was none: it runs the full width of the mouth and stands about as tall as the upper lip is thick, obvious at a glance rather than a sliver. The lower lip draws back and tucks in under that band so that its coloured area ends up clearly thinner than the upper lip\'s. The teeth sit against the lip along their whole width, and the mouth is not open: there is no dark gap, no dark opening and no dark shadow anywhere between the teeth and the lower lip, and no part of the inside of the mouth is visible. The mouth also stays as wide as it was, with the corners where they were and no smile or spread. The result has to be plainly distinguishable from a relaxed closed mouth, because a clear white band of teeth crosses the whole width of the mouth and the lower lip is visibly thinner; and from a parted mouth with teeth showing, because the only shapes here are lip, teeth and lip, with nothing dark between them.',
      FV_NOTE,
    ),
  },
  /*
